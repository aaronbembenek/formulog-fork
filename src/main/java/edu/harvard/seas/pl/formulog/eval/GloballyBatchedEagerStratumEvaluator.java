...
public final class GloballyBatchedEagerStratumEvaluator extends AbstractStratumEvaluator {
	...
	Map<RelationSymbol, AtomicBoolean> locks = new HashMap<>();

	static final int taskSize = Configuration.taskSize;
	static final int smtTaskSize = Configuration.smtTaskSize;

	public GloballyBatchedEagerStratumEvaluator(int stratumNum, SortedIndexedFactDb db, Iterable<IndexedRule> rules,
		...
		for (RelationSymbol sym : laterRoundRules.keySet()) {
			locks.put(sym, new AtomicBoolean());
		}
	}

	private Map<RelationSymbol, Set<Term[]>> resetDeltaSets() {
		var r = deltaSets;
		deltaSets = new ConcurrentHashMap<>();
		for (RelationSymbol sym : laterRoundRules.keySet()) {
			...
			for (var e : resetDeltaSets().entrySet()) {
				...
				if (!s.isEmpty()) {
					for (var r : laterRoundRules.get(e.getKey())) {
						exec.externallyAddTask(new RulePrefixEvaluator(r, s));
						didSomething = true;
					}
				}
			}
	...
	void addToDeltaSet(RelationSymbol sym, Term[] tup) {
		var s = deltaSets.get(sym);
		s.add(tup);
		if (s.size() > batchSize) {
			var lock = locks.get(sym);
			if (lock.compareAndSet(false, true)) {
				deltaSets.put(sym, Util.concurrentSet());
				for (var r : laterRoundRules.get(sym)) {
					exec.externallyAddTask(new RulePrefixEvaluator(r, s));
				}
				lock.set(false);
			}
	...
	class RulePrefixEvaluator extends AbstractFJPTask {
		...
		protected RulePrefixEvaluator(IndexedRule rule, Iterable<Term[]> deltaArgs) {
			super(exec);
			this.rule = rule;
